    ...
    if options.layer:
        ...
        for layer in options.layer.split(","):
            ...
            for year in range(year_from,year_to):
                output += "buildout/bin/tilemanager -t 64 -c etc/tilecache.swisstopo.zeitreihen.zoomlevel20.1965-2010.%s.cfg %s 20 21 -b 655000,194000,672500,206000\n" % (year,layer)
            output +="\n# Start tile generation for layer %s ...\n" % layer
            for year in range(year_from,year_to):
                output += "buildout/bin/tilemanager -t 64 -c etc/tilecache.swisstopo.zeitreihen.zoomlevel20.1965-2010.%s.cfg %s 20 21\n" % (year,layer)
            output +="\n\n"
        print output
