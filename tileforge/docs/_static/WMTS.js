...
OpenLayers.Layer.WMTS = OpenLayers.Class(OpenLayers.Layer.Grid, {
    ...
    getURL: function(bounds) {
        ...
        if (!this.tileFullExtent || this.tileFullExtent.intersectsBounds(bounds)) {            
            var info = this.getTileInfo(bounds.getCenterLonLat());
            if (this.requestEncoding.toUpperCase() === "REST") {
                
                if (this.resourceUrl) {
                    url = this.resourceUrl;
                    url = url.replace(/{style}/g, this.style);
                    // FIXME: dimension
                    url = url.replace(/{TileMatrixSet}/g, this.matrixSet);
                    url = url.replace(/{TileMatrix}/g, this.matrix.identifier);
                    url = url.replace(/{TileRow}/g, info.row);
                    url = url.replace(/{TileCol}/g, info.col);
                } else {
